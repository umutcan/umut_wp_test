...
?>
<a href="<?php echo substr(get_permalink(),0,-1).'/';?>">Proje  Sayfasına Dön</a><br/>
<br/>
...
if(isset ($_REQUEST["pid"])&& $_REQUEST["pid"]>0) {
        ...
        foreach ($_POST as $key=>$value){
            ...
            if($value!=NULL){
                ...
                if($wpdb->update("subtitle",array("texttr"=>$value,"rev"=>$rev_info["rev"]),array("project_id"=>$pid,"line"=>$key)))
                        echo "Gönderdikleriniz sisteme başarı ile eklendi.<br>";//$key."->".stripslashes ($value)."<br/>";
                else
                    echo "";
                }
            }
    ...
    echo "< <a href='".  get_permalink()."?pid=$pid&paging="
    ...
    if(!$result)
        echo "Olmadı!";
    echo "<form class='frm' action='".get_permalink()."?pid=$pid&paging=$page' method='post' >";
    echo "<input type='submit' /><br/>";
    foreach ($result as $row) {
        echo "<div class='sline'>";
        echo $row->line."<br>".$row->text."<br>".$row->texttr."<br>";
        echo "<textarea id='$row->line' name='$row->line' rows='1' cols='80' >".stripslashes($row->texttr)."</textarea><br>";
        echo "</div>";
